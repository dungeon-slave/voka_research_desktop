...
// Достаем глобальные окна, инициализированные во flutter_window.cpp
extern HWND g_unity_hwnd;
extern HWND g_flutter_child_hwnd; // ИСПРАВЛЕНО: теперь имя совпадает!
...
LRESULT Win32Window::MessageHandler(
  ...
  switch (message) {
    case WM_SIZE: {
        ...
        // 1. Unity на фоне (на весь экран)
        if (g_unity_hwnd && IsWindow(g_unity_hwnd)) {
            MoveWindow(g_unity_hwnd, 0, 0, width, height, TRUE);
        }

        // 2. Flutter поверх Unity (тоже на весь экран)
        if (g_flutter_child_hwnd && IsWindow(g_flutter_child_hwnd)) {
            MoveWindow(g_flutter_child_hwnd, 0, 0, width, height, TRUE);
            
            // Жестко прибиваем Flutter на самый верх Z-индекса среди детей
            SetWindowPos(g_flutter_child_hwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
        }
        
        // ВАЖНО: Не делаем return 0; здесь, пробрасываем WM_SIZE дальше, 
        // чтобы Flutter обновил свой внутренний layout!
        break; 
    }
